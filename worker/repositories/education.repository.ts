...
// Degrees & qualifications data access with atomic D1 batch mutation & revision auditing
// ================================================================
...
export class EducationRepository {
  ...
  async getAllPublic(): Promise<EducationRecord[]> {
  ...
  async getById(id: string): Promise<EducationRecord | null> {
    return await this.db
      .prepare('SELECT * FROM education WHERE id = ?')
      .bind(id)
      .first<EducationRecord>();
  }

  prepareInsertStatement(
    data: Omit<EducationRecord, 'version' | 'created_at' | 'updated_at'>,
    now: string
  ): D1PreparedStatement {
    return this.db
      .prepare(`
  ...
  prepareUpdateStatement(
    id: string,
    data: Omit<EducationRecord, 'id' | 'version' | 'created_at' | 'updated_at'>,
  ...
  async createWithRevision(
    ...
    const revStmt = revRepo.createRevisionStatement({
      id: `rev-education-${data.id}-1-${Date.now()}`,
  ...
  async updateWithRevision(
    ...
    const revStmt = revRepo.createConditionalRevisionStatement({
      id: `rev-education-${id}-${nextVersion}-${Date.now()}`,
      entity_type: 'education',
      entity_id: id,
      version: nextVersion,
      action: 'update',
  ...
  async create(data: Omit<EducationRecord, 'version' | 'created_at' | 'updated_at'>): Promise<EducationRecord> {

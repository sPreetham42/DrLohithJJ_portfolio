...
export class SessionRepository {
   ...
   */
  async createSession(data: CreateSessionInput): Promise<AdminSessionRecord> {
   ...
   */
  async getSessionByTokenHash(tokenHash: string, nowIso?: string): Promise<AdminSessionRecord | null> {
   ...
   */
  async updateLastUsedAt(id: string, nowIso?: string): Promise<void> {
    const now = nowIso || new Date().toISOString();
    await this.db
      .prepare(`
   ...
   */
  async revokeSessionByTokenHash(tokenHash: string, nowIso?: string): Promise<void> {
    const now = nowIso || new Date().toISOString();
    await this.db
      .prepare(`
        UPDATE admin_sessions

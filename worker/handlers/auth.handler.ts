// ================================================================
// GITHUB OAUTH & SESSION HANDLERS
// Secure OAuth 2.0 authorization, state CSRF verification, and D1 session management
// ================================================================

import { Env } from '../types';
import { UnauthorizedError, ForbiddenError, ValidationError, ApiError } from '../errors';
import { SessionRepository, hashSessionToken } from '../repositories/session.repository';
import { getNoCacheHeaders } from '../middleware/cache';
 ...
 */
export async function handleAuthGithubLogin(request: Request, env: Env): Promise<Response> {
 ...
 */
export async function handleAuthGithubCallback(request: Request, env: Env): Promise<Response> {
  const clientId = env.GITHUB_CLIENT_ID;
  ...
  let userEmail: string | null = githubUser.email || null;
  try {
    ...
    // Non-fatal if emails endpoint fails; fallback to profile email
  }

  // 5. Authoritative Identity & Allowlist Enforcement
  // Allowlist can contain numeric GitHub IDs, usernames, or admin emails
